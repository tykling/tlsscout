...
def StartScan(sitecheck):
    print "StartScan called for site %s" % sitecheck.site.hostname
    ...
    if sitecheck.urgent or not urgentonly:
        ...
        print "this sitecheck can not be started now"


def GetResults(sitecheck):
    hostinfo = Analyze(host=sitecheck.site.hostname, publish="on" if sitecheck.site.group.publish else "off", all="done", sitecheck=sitecheck)
    return hostinfo
